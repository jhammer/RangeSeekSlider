...
#warning("need to fix ax")
@IBDesignable open class RangeSeekSlider: NSView, CALayerDelegate, NSViewLayerContentScaleDelegate, NSAccessibilityGroup {
    ...
    open var minLabelFont: NSFont = NSFont.systemFont(ofSize: 12.0) {
        didSet {
    ...
    open var maxLabelFont: NSFont = NSFont.systemFont(ofSize: 12.0) {
        didSet {
    ...
    @IBInspectable open var handleImage: NSImage? {
        didSet {
    ...
    private var leftHandleAccessibilityElement: NSAccessibilitySlider {
        return RangeSeekSliderAccessibilityElement(slider: self,
                                                   label: minLabelAccessibilityLabel,
                                                   value: minLabel.string as? String,
                                                   frame: convert(leftHandle.frame, to: nil),
                                                   selectedValueKeyPath: \RangeSeekSlider.selectedMinValue,
                                                   labelKeyPath: \RangeSeekSlider.minLabel)
    }

    private var rightHandleAccessibilityElement: NSAccessibilitySlider {
        return RangeSeekSliderAccessibilityElement(slider: self,
                                                   label: maxLabelAccessibilityLabel,
                                                   value: maxLabel.string as? String,
                                                   frame: convert(rightHandle.frame, to: nil),
                                                   selectedValueKeyPath: \RangeSeekSlider.selectedMaxValue,
                                                   labelKeyPath: \RangeSeekSlider.maxLabel)
    }
    ...
    open override var intrinsicContentSize: NSSize {
        return NSSize(width: NSView.noIntrinsicMetric, height: 65.0)
    }
    ...
    open override var acceptsFirstResponder: Bool {
        ...
        return NSApp.isFullKeyboardAccessEnabled && window?.keyViewSelectionDirection != .directSelection
    }
    
    open override func becomeFirstResponder() -> Bool {
        switch window?.keyViewSelectionDirection {
        case .directSelection: fallthrough
        case .selectingNext:
            keyHandle = leftHandle
        case .selectingPrevious:
            keyHandle = rightHandle
        default: break
        }
        return true
    }
    ...
    open override func drawFocusRingMask() {
        guard let keyHandle = keyHandle else { return }
        NSBezierPath(ovalIn: keyHandle.frame).fill()
    }
    
    open override var focusRingMaskBounds: NSRect {
        guard let keyHandle = keyHandle, handleTracking == .none else {
            return .zero
        }
        return keyHandle.frame
    }
    
    open override func keyDown(with event: NSEvent) {
        guard let keyHandle = keyHandle,
              let char = event.charactersIgnoringModifiers?.unicodeScalars.first?.value else {
            return
        }
        
        func adjust(by step: CGFloat) {
            switch keyHandle {
            case leftHandle: selectedMinValue += step
            case rightHandle: selectedMaxValue += step
            default: break
            }
            refresh()
        }
        ...
        switch Int(char) {
        case NSLeftArrowFunctionKey: fallthrough
        case NSDownArrowFunctionKey:
            adjust(by: -step)
        case NSRightArrowFunctionKey: fallthrough
    ...
    open override func mouseDown(with event: NSEvent) {
        let touchLocation: CGPoint = self.convert(event.locationInWindow, from: nil)
    ...
    private func setup() {
        ...
        minLabelFont = NSFont.systemFont(ofSize: labelFontSize)
        ...
        maxLabelFont = NSFont.systemFont(ofSize: labelFontSize)
    ...
    @objc open func layer(_ layer: CALayer, shouldInheritContentsScale newScale: CGFloat, from window: NSWindow) -> Bool {
        return true
    }
    ...
    private func updateColors() {
        ...
        if let initialColor = initialColor?.cgColor, isInitial {
            ...
            let color: CGColor = (handleImage == nil) ? initialColor : NSColor.clear.cgColor
            ...
            let tintCGColor: CGColor = NSColor.controlAccentColor.cgColor
    ...
    private func updateAccessibilityElements() {
        self.setAccessibilityChildren([leftHandleAccessibilityElement, rightHandleAccessibilityElement])
    }
...
// MARK: - RangeSeekSliderAccessibilityElement

private final class RangeSeekSliderAccessibilityElement: NSObject, NSAccessibilitySlider {
    weak var slider: RangeSeekSlider?
    var label: String?
    var value: String?
    let frame: NSRect
    let selectedValueKeyPath: ReferenceWritableKeyPath<RangeSeekSlider, CGFloat>
    let labelKeyPath: KeyPath<RangeSeekSlider, CATextLayer>
    
    init(slider: RangeSeekSlider, label: String?,
         value: String?,
         frame: NSRect,
         selectedValueKeyPath: ReferenceWritableKeyPath<RangeSeekSlider, CGFloat>,
         labelKeyPath: KeyPath<RangeSeekSlider, CATextLayer>) {
        self.slider = slider
        self.label = label
        self.value = value
        self.frame = frame
        self.selectedValueKeyPath = selectedValueKeyPath
        self.labelKeyPath = labelKeyPath
        super.init()
    }
    
    func accessibilityLabel() -> String? { label }
    func accessibilityValue() -> Any? { value }
    
    func accessibilityPerformIncrement() -> Bool {
        guard let slider = slider else { return false }
        slider[keyPath: self.selectedValueKeyPath] += slider.step
        value = slider[keyPath: self.labelKeyPath].string as? String
        return true
    }
    ...
    func accessibilityFrame() -> NSRect { frame }
    func accessibilityParent() -> Any? { slider }
}
